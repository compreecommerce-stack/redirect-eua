import './style.css';
import { PRESELL_CONFIG } from './config.js';
...
(function() {
  ...
  async function handleVerification(event) {
    ...
    if (!humanCheck.isHuman) {
      if (PRESELL_CONFIG.security && PRESELL_CONFIG.security.showSafePageForBots) {
        activateSafePage();
      } else {
    ...
    showStatus((PRESELL_CONFIG.security && PRESELL_CONFIG.security.verifyingText) || 'Verifying...', '');

    trackEvent('ViewContent', { content_name: 'Captcha_Verified' });

    const verificationDelay = PRESELL_CONFIG.redirectDelayMs || 800;
    ...
    setTimeout(() => {
      ...
      showStatus((PRESELL_CONFIG.security && PRESELL_CONFIG.security.successText) || 'Verified! Redirecting...', 'success');
  ...
  async function runSecurityAudit() {
    ...
    if (PRESELL_CONFIG.security && PRESELL_CONFIG.security.enableIpDatacenterCheck) {
      AntiBot.checkDatacenterIP().then(result => {
        if (result.isBot && PRESELL_CONFIG.security && PRESELL_CONFIG.security.showSafePageForBots) {
          activateSafePage();
        }
      });
    }
