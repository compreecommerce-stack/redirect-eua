const AntiBot = (function() {
  ...
  function checkUserAgent() {
    ...
    const botKeywords = [
      ...
      'go-http-client', 'spider', 'crawler', 'scraper', 'bot', 'crawl',
      'phantom', 'nightmare', 'cypress', 'electron', 'postman', 'pipedream',
      'adreview', 'mediapartners', 'adsbot', 'byte-spider', 'bytedance-review'
    ];
  ...
  function checkAutomation() {
    ...
    for (let i = 0; i < automationGlobals.length; i++) {
      if (automationGlobals[i] in window || automationGlobals[i] in document) {
  ...
  function checkWebGLRenderer() {
    try {
      ...
      const emulatedRenderers = [
        ...
        'vmware', 'software rasterizer', 'basic render driver',
  ...
  function checkMobileConsistency() {
    ...
    if (isMobileUA) {
      ...
      if (!hasTouch) {
        return { isBot: true, reason: 'MOBILE_UA_NO_TOUCH' };
      }

      const platform = (navigator.platform || '').toLowerCase();
      if (ua.includes('iphone') && (platform.includes('win') || platform.includes('linux'))) {
        return { isBot: true, reason: 'SPOOFED_IOS_PLATFORM_' + platform };
      }
    }
  ...
  function checkHeadlessEnvironment() {
    ...
    if (window.screen && window.screen.width === 800 && window.screen.height === 600 && window.outerWidth === 800) {
      return { isBot: true, reason: 'HEADLESS_DEFAULT_RES' };
    }
    ...
    const isChrome = ua.includes('chrome') && !ua.includes('crios') && !ua.includes('edge') && !ua.includes('edg');
    if (isChrome && !window.chrome) {
      return { isBot: true, reason: 'CHROME_NO_OBJECT' };
    }

    try {
      if (window.Notification && Notification.permission === 'denied' && !navigator.permissions) {
        return { isBot: true, reason: 'NOTIFICATION_ANOMALY' };
      }
    } catch (e) {}
  ...
  function validateHumanInteraction(clickEvent) {
    ...
    if (pointerEventsCount === 0 && elapsed < 2000) {
      return { isHuman: false, reason: 'ZERO_POINTER_ACTIVITY' };
    }
  ...
  async function checkDatacenterIP() {
    const hostingTerms = [
      ...
      'oracle', 'cloudflare', 'hosting', 'datacenter', 'servers', 'vps',
      'leaseweb', 'choopa', 'vultr', 'fastly', 'colocation', 'dedicated',
      'akamai', 'm247', 'cogent', 'rackspace', 'contabo'
    ];
